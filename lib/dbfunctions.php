...
class dbobject 
{
	...
	function SaveTransEdit($tbl,$inpFds,$inpFdsVals,$operation)
	{
		...
		//file_put_contents('aa.txt',$operation);
		$whrcond = 0;
		$resp = 0;
		if($operation == 'new'){
		...
		$query2 = "insert into audit_logg  values('','$tbl','$trail_appl','New','$daty','$officer','$ip')";
		...
		$query_data = $query.$where;
		$query_data .=';';
		//file_put_contents('aaa.txt',$query_data);
		$daty = @date('Y-m-d H:i:s');
		$officer = $_SESSION['username_sess'];
		$ip = $_SERVER['REMOTE_ADDR'];
		$query2 = "insert into audit_logg values('','$tbl','$trail_appl','Edit','$daty','$officer','$ip')";
		if(mysql_query($query_data))//or die(mysql_error()))
		{
			$resp += 1;
			if(mysql_query($query2));
...
// END NEW ADDITIONS	
	...
	////////////////////////
	function doAsset($app_id,$asset_class,$asset_name,$created,$officer){
			$count_entry = 0;
			$query = "select * from assets_setup  where app_id='$app_id'";
			//echo $query;
			$result = mysql_query($query);
			$numrows = mysql_num_rows($result);
			if($numrows >=1){
			$query_data ="update assets_setup set app_id='$app_id', asset_class='$asset_class' , asset_name='$asset_name' , created='$created' , officer='$officer' where app_id='$app_id' ";
			$result_data = mysql_query($query_data);
			$count_entry =mysql_affected_rows();
			...
			{
			$sql = "select * from assets_setup where app_id='$app_id'";
			if($res=mysql_query($sql))
			{
			if(mysql_num_rows($res)>=1)
			{
			...
			{	
			$query_data = "insert into assets_setup(app_id,asset_class,asset_name,created,officer) values( '$app_id','$asset_class','$asset_name',now(),'$officer')";
			//echo $query_data;
			$result_data = mysql_query($query_data);
			$count_entry = mysql_affected_rows();
			}
	...
	function doUser($operation,$username,$userpassword,$user_id,$dept_id,$firstname,$lastname,$email,$phone,$chgpword_logon,$user_locked, $user_disable,$day_1,$day_2,$day_3,$day_4,$day_5,$day_6,$day_7,$override_wh,$extend_wh,$role_id,$role_name,$posted_user)//, $profile_pic
	{
				$desencrypt = new DESEncryption();
				$count_entry = 0;
				$key = $username;
				...
				if($numrows >=1 && $operation!='new')
				{
						...
						$query_data ="update userdata set password='$str_cipher_password', user_id='$user_id',dept_id='$dept_id', role_id='$role_id',firstname='$firstname', lastname='$lastname', email='$email', mobile_phone='$phone', passchg_logon='$chgpword_logon', user_disabled='$user_disable', user_locked='$user_locked', day_1='$day_1', day_2='$day_2', day_3='$day_3', day_4='$day_4', day_5='$day_5', day_6='$day_6', day_7='$day_7', modified=now(), override_wh='$override_wh', extend_wh='$extend_wh',posted_user='$posted_user',last_used_passwords='$LastUsedPassword' where username='$username'";  // , profile_pic='$profile_pic'
						//echo $query_data;
		 	...
		 	$query_data = "insert into userdata (username,password,user_id,dept_id,role_id, firstname, lastname, email, mobile_phone, passchg_logon, user_disabled, user_locked,day_1,day_2,day_3,day_4,day_5,day_6,day_7,created, modified,override_wh,extend_wh,pass_dateexpire , posted_user ) values( '$username','$str_cipher_password','$user_id','$dept_id','$role_id', '$firstname','$lastname','$email','$phone','$chgpword_logon','$user_disable','$user_locked','$day_1', '$day_2', '$day_3', '$day_4', '$day_5', '$day_6', '$day_7' , now(), now(), '$override_wh', '$extend_wh', '$pass_dateexpire' , '$posted_user')"; //, profile_pic , '$profile_pic'
						echo $query_data;
						$result_data = mysql_query($query_data); //or die(mysql_error());
						$count_entry = mysql_affected_rows();
				} //End inner else
	...
	/////////////////////////////////////////////////////////
	function getproject($opt) {
	$filter = "";
	$options = "<option value='#'>::: Select Project Option ::: </option>";
		 if($opt!= ""){
		 $filter = " and project_id='".$opt."' "; //" username='$username' and password='$password' ";
		 }
		 $filter .=" order by project_name ";
		 $dbobject = new dbobject();
	 $user_role_session = $_SESSION['role_id_sess'];
	 //$filter_role_id = $dbobject->getitemlabel('parameter','parameter_name','admin_code','parameter_value');
	 //$filter_menu_id = $dbobject->getitemlabelmenu('parameter','parameter_name','admin_menu_code','parameter_value');
	 //$filteradmin = ($user_role_session == $filter_role_id)?"":" and menu_id not in (".$filter_menu_id.")";
	$query = "select distinct project_id, project_name from project where 1=1 ".$filter;
	//echo $query;
	$result = mysql_query($query);
	$numrows = @mysql_num_rows($result);
	if($numrows > 0){
		...
		$row = mysql_fetch_array($result);
		//echo $row['country_code'];
		 if($opt==$row['project_id']) $filter='selected';
		//echo ($opt=='$row["country_code"]'?'selected':'None');
		$options = $options."<option value='$row[project_id]' $filter >$row[project_name]</option>";
		$filter='';
		}
	}
	return $options;
	...
	function doMenuGroup($menu_id,$exist_role){
		...
		/////////////////////////////////
	function getexistuser($opt) {
	$filter = "";
	$username_session = $_SESSION['username_sess'];
	//$options = "<option value='#'>::: Select Menu Option ::: </option>";
		 if($opt!= ""){
		 $filter = "where project_id='$opt'"; //" username='$username' and password='$password' ";
		 }
	$query = "select user_id, username from userdata where user_id in (select user_id from projectgroup ".$filter.") and username not in(select parameter_value from parameter where parameter_name='$username_session')";
	//echo $query;
	...
	if($numrows > 0){
		...
		//echo $row['country_code'];
		 //if($opt==$row['role_id']) $filter='selected';
		//echo ($opt=='$row["country_code"]'?'selected':'None');
		$options = $options."<option value='$row[user_id]' $filter >$row[username]</option>";
		$filter='';
		}
	}
	return $options;
	...
	$query = "select user_id, username from userdata where user_id not in (select user_id from projectgroup   ".$filter.") and username not in(select parameter_value from parameter where parameter_name='$username_session' )";
	...
	function doProjectGroup($project_id,$exist_user){
			$count_entry = 0;
			$exist_user_arr = explode(",",$exist_user);
			$user_id = "";
			for($i=0; $i<count($exist_user_arr); $i++){
			$user_id = $user_id."'".$exist_user_arr[$i]."', ";
			}
			$user_id = substr($user_id,0,(strlen($user_id)-2));
			$query_data ="delete from projectgroup where user_id not in ($user_id, 001) and project_id='$project_id' ";
			//echo $query_data.'<br>';
			$result_data = mysql_query($query_data);
			$count_entry += mysql_affected_rows();

			for($i=0; $i<count($exist_user_arr); $i++){
		   $query_data_i = "insert into projectgroup values ('$exist_user_arr[$i]','$project_id')";
			//echo $query_data_i.'<br>';
			$result_data_i = mysql_query($query_data_i);
			$count_entry += mysql_affected_rows($dbconn); //by abigail
			}
			//echo "Count Entry :: ".$count_entry;
			return $count_entry;
		}
...
//////////////////////////Beginning of Isaiah///////////////////////////////
	...
	function getTableSelectArr($tablename,$selarr,$whrarr,$whrvalarr,$order,$orderdir,$opt,$initOpt)
	{
		...
		//file_put_contents('que.txt',$query);
		//echo $query.'-'.$opt;
		$result = mysql_query($query);
		$numrows = mysql_num_rows($result);
		if($numrows > 0){
