	...
	function callPageEdit(str,pgload,divd)
{
	...
	if(data!='error')
	{
		$.ajax({
			...
			{ 
				...
				if(str=='project')
				{
				getpage('project_list.php','page');
				} else
				
				if(str=='dept')
				{
				getpage('dept_list.php','page');
				} else
				
				if(str=='client')
				{
					getpagecust('client_list.php','page');
				} else
				if(str=='system')
				{
					getpage('system_list.php','page');
				} else
				
				if(pgload=='target.php')
				{
				getpage('project_log_staff.php','page');
				}else
				
				if(pgload=='work_done.php')
				{
				getpage('project_log_staff.php','page');
				}else
				
				if(str=='daily_progress')
				{
				getpage('project_log.php','page');
				}else{ 
				alert('add me to ur if else statement');
				}
 ...
 function callPageEditcust(str,pgload,divd)
{
	var operation = $("#operation").html(),
		p_id = $("#project_id-fd").val();
	...
	if(data!='error')
	{
		$.ajax({
			...
			{ 
				...
				if(str=='deliverables')
				{
					getpagecust('deliverables_list.php?op=ree','page','project_id='+p_id);
				} else
...
function getpagecust(str,divid,params) 
{ 
	//var data = getdata();
	//alert(data);
			/*
...
function loaduser(){
	var data = escape($('#project_id').val());
   $.ajax({ 
	   type: "POST", 
	   url: "utilities.php", 
	   data: "op=getnonexistuser&project_id="+data, 
	   success: function(msg){ 
		 //alert( "Data Saved: " + msg ); 
		 //alert(data);
		 $("#non_exist_user").html(msg);
		 //$("#display_message").show("fast");
	   } 
  });
   // for existing roles
   ...
   $.ajax({ 
	   type: "POST", 
	   url: "utilities.php", 
	   data: "op=getexistuser&project_id="+data, 
	   success: function(msg){ 
		 //alert( "Data Saved: " + msg ); 
		 //alert(data);
...
function adduser(){
	return !$('#non_exist_user option:selected').remove().appendTo('#exist_user'); 
}
function removeuser(){
	return !$('#exist_user option:selected').remove().appendTo('#non_exist_user');
}  
function addrole(){
...
function selectalldataa(){
	$("#exist_user *").attr("selected","selected");
}
