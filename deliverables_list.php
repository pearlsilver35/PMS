...
<?php
	...
	{
		$query = "select * from deliverables where project_id='".$project_id."' and project_id not in(select parameter_value from parameter where parameter_name='501' )".$filter.$searchdate.$order.$limit_filter;
	}
	//echo $query;
...
<table width="100%">
  ...
  <form class="mws-form" action="" method="post" id="form1">
   <input type="text" name="deliverable-fd" id="deliverable-fd" class="required-text form-control" title="Deliverable" value="<?php echo $deliverable; ?>" placeholder="Deliverable" />&nbsp;&nbsp;
    ...
    <input type="button" class="btn" name="subbtn" id="subbtn" value="<?php 
	if($op =='edit'){
	 ...
	 ?>" onclick="javascript: calldage()" class="btn btn-info">&nbsp;&nbsp;
                  </form>
...
<td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["deliverable_id"];?></td>
  ...
  <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["officer"];?></td> 
        ...
        <td nowrap="nowrap"><a href="javascript:getpage('deliverables_list.php?op=edit&amp;deliverable_id=<?php echo $row["deliverable_id"];?>','page')"><font color="#33CC00">Edit</font></a>
       &nbsp|&nbsp;<a href="javascript:getpage('deliverable_list.php?op=del&amp;deliverable_id=<?php echo $row["deliverable_id"];?>','page')"><font color="#FF0000">Delete</font></a>
     </td> </tr>
      <?php 
				} //End If Result Test	
		  ?>
  ...
  function calldage()
  {         
	 	callPageEditcust('deliverables','deliverables_list.php','page');
