...
<?php
			...
			$searchby = !isset($_REQUEST['searchby'])?"dept_id":$_REQUEST['searchby'];
			$keyword = !isset($_REQUEST['keyword'])?"":$_REQUEST['keyword'];
			$orderby = !isset($_REQUEST['orderby'])?"dept_id":$_REQUEST['orderby'];
		  ...
		  $sql = "select count(dept_id) counter from dept where 1=1 and dept_id not in(select parameter_value from parameter where parameter_name='501' )".$filter.$searchdate;	// Filter admin
			//echo $sql;
			...
			if ($sel=="del"){
				$delvalues = !isset($_REQUEST['dept_id'])?"":$_REQUEST['dept_id'];
				$delsql = "delete from dept where dept_id = '$delvalues'";
				...
				if($delvalues!=''){
					...
					$delsql = "delete from dept where dept_id in ($delvalues)";	
					//echo $delsql;
					$dresult = mysql_query($delsql);
				}
	...
	if($_SESSION['project_id_sess']=='001' || $_SESSION['project_id_sess']=='501')
	{
		...
		$query = "select * from dept where 1=1 ".$filter.$searchdate.$order.$limit_filter;
	}else
	{
		$query = "select * from dept where 1=1 and dept_id not in(select parameter_value from parameter where parameter_name='501' )".$filter.$searchdate.$order.$limit_filter;
	}
	//echo $query;
	...
	<td width="1116" rowspan="2" align="left"><table width="507">
	  <tbody><tr>
	    ...
	    <td width="25%"><select name="searchby" class="table_text1"  id="searchby">
	      <option value="dept_id" <?php echo ($searchby=='dept_id')?"selected":""; ?>>Department ID</option>
	        <option value="dept_name" <?php echo ($searchby=='dept_name')?"selected":""; ?>>Department Name</option>
	      </select></td>
	    ...
	    <td width="33%">
	      <input name="keyword" type="text" class="table_text1" id="keyword" value=""></td>  <td width="14%"><span class="submit">
	        <input name="search2" type="button" class="btn" id="search2" onclick="javascript:doSearch('dept_list.php')" value="Search"></span>
	        </td></tr>
	  <tr>
	    <td align="right">Order:</td>
	    <td colspan="4">
        <select name="orderby" class="table_text1"  id="orderby">
                  <option value="dept_id" <?php echo ($orderby=='dept_id')?"selected":""; ?>>Department ID</option>
                  <option value="asset_name" <?php echo ($orderby=='dept_name')?"selected":""; ?>>Department Name</option>
                  </select>
...
<tr>
  ...
  <td align="right" nowrap="nowrap"> Limit :
    <select name="limit"   id="limit" onchange="javascript:doSearch('dept_list.php');" style="width:100px;">
    ...
    &nbsp; </td>
  <td width="271" align="left"><input name="search22" type="button" class="btn" id="search22" onclick="javascript:doSearch('dept_list.php');" value="Go" /></td>
  <td width="49" colspan="-2" align="right"><span>&nbsp;</span></td>
  <td width="186" align="left">&nbsp;</td>
  <td width="34" colspan="-2"><span class="table_text1"><a href="#" onclick="javascript: goFirst('dept_list.php');"><img src="images/first.png" alt="First" width="24" height="24" border="0" /></a></span></td>
  <td width="32" colspan="-2"><span class="table_text1"><a href="#" onclick="javascript: goPrevious('dept_list.php');"><img src="images/prev.png" alt="Previous" width="24" height="24" border="0" /></a></span></td>
  <td width="32" colspan="-2"><span class="table_text1"><a href="#" onclick="javascript: goNext('dept_list.php');"><img src="images/next.png" alt="Next" width="24" height="24" border="0"/></a></span></td>
  <td width="63" colspan="-2"><span class="table_text1"><a href="#" onclick="javascript: goLast('dept_list.php');"><img src="images/last.png" alt="Last" width="24" height="24" border="0"/></a></span></td>
</tr>
</table>
...
<table width="100%">
  <tr>
  &nbsp;&nbsp;<input name="add_new" type="button" class="btn" id="search" onclick="getpage('dept.php','page')" value="Add New">
    ...
    <table class="table" width="100%">
      <thead>
        <tr class="ewTableHeader">
          ...
          <th nowrap="nowrap" class="Odd">Department ID</th>
          <th nowrap="nowrap" class="Odd">Department Name</th>
          <th nowrap="nowrap" class="Odd">H.O.D</th>
      ...
      <tr <?php echo ($k%2==0)?"class='treven'":"class='trodd'"; ?> >
        ...
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["dept_id"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["dept_name"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["dept_hod"];?></td>
        ...
        <td nowrap="nowrap"><a href="javascript:getpage('dept.php?op=edit&amp;dept_id=<?php echo $row["dept_id"];?>','page')"><font color="#33CC00">Edit</font></a>&nbsp;|&nbsp;<a href="javascript:getpage('dept_list.php?op=del&amp;dept_id=<?php echo $row["dept_id"];?>','page')"><font color="#FF0000">Delete</font></a></td>
      </tr>
      <?php 
				} //End If Result Test	
		  ?>
