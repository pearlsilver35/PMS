...
}elseif($op=='save_asset')
{
		...
		$asset_resp = $dbobject->doAsset($app_id,$asset_class,$asset_name,$created,$officer);
			if($asset_resp=='1') {
				echo 'Asset detail has been successfully saved';
			}else{
				echo 'Error : Please check Asset detail';
			}
		//}
...
/*if($_FILES['imager']['name'] == ""){
		$target = "img/users/aqvatarius.jpg";
		
	}else{
	$target = "img/users/".basename($_FILES['imager']['name']);
	...
	if (move_uploaded_file($_FILES['imager']['tmp_name'], $target)) {
		...
		*/
		...
		$user_id=$dept_id."/".$dbobject->paddZeros($dbobject->getnextid('userdata'),3);
		...
		$posted_user = $_REQUEST['posted_user'];
		//$profile_pic = $target;
		$insurance_coy = $_REQUEST['insurance_coy'];
		
		$user_resp = $dbobject->doUser($operation,$username,$userpassword,$user_id,$dept_id,$firstname,$lastname,$email,$phone, $chgpword_logon, $user_locked, $user_disable,$day_1,$day_2,$day_3,$day_4,$day_5,$day_6,$day_7,$override_wh,$extend_wh,$role_id,$role_name,$posted_user); //,$profile_pic
	...
	if($op=='getexistuser'){
			...
			$existuser = $dbobject->getexistuser($project_id);
			echo $existuser;
	}
	////////////////////////////////////
	if($op=='getnonexistuser'){
			$project_id = $_REQUEST['project_id'];
			$nonexistuser = $dbobject->getnonexistuser($project_id);
			echo $nonexistuser;
			
	}elseif($op=='save_projectgroup'){
		if(isset($_REQUEST['project_id'])){
		$project_id = $_REQUEST['project_id'];
		$exist_user = $_REQUEST['exist_user'];
		$projectgroup_resp = $dbobject->doProjectGroup($project_id,$exist_user);
			if($projectgroup_resp > 0) {
				echo 'Project Group detail has been successfully saved';
			}else{
				echo 'Project Group detail has been successfully saved.'; //Error : Please check Project Group detail
			}
		}
	}
	...
	}elseif ($op=="editTrans")
{
	...
	//{	
		...
		//file_put_contents('aaa.txt', $operation);
