...
<?php
			...
			$searchby = !isset($_REQUEST['searchby'])?"project_id":$_REQUEST['searchby'];
			$keyword = !isset($_REQUEST['keyword'])?"":$_REQUEST['keyword'];
			$orderby = !isset($_REQUEST['orderby'])?"project_id":$_REQUEST['orderby'];
			...
			$sql = "select count(project_id) counter from assets_setup where 1=1 and project_id not in(select parameter_value from parameter where parameter_name='501' )".$filter.$searchdate;	// Filter admin
			//echo $sql;
			...
			if ($sel=="del"){
				$delvalues = !isset($_REQUEST['project_id'])?"":$_REQUEST['project_id'];
				$delsql = "delete from project where project_id = '$delvalues'";
				...
				if($delvalues!=''){
					...
					$delsql = "delete from project where project_id in ($delvalues)";	
					//echo $delsql;
					$dresult = mysql_query($delsql);
				}
	...
	if($_SESSION['project_id_sess']=='001' || $_SESSION['project_id_sess']=='501')
	{
		...
		$query = "select * from project where 1=1 ".$filter.$searchdate.$order.$limit_filter;
	}else
	{
		$query = "select * from project where 1=1 and project_id not in(select parameter_value from parameter where parameter_name='501' )".$filter.$searchdate.$order.$limit_filter;
	}
	//echo $query;
	  ...
	  <tbody><tr>
	    ...
	    <td width="25%"><select name="searchby" class="table_text1"  id="searchby">
	      <option value="project_id" <?php echo ($searchby=='project_id')?"selected":""; ?>>Project ID</option>
	        <option value="project_name" <?php echo ($searchby=='project_name')?"selected":""; ?>>Project Name</option>
	      </select></td>
	    ...
	    <td width="33%">
	      <input name="keyword" type="text" class="table_text1" id="keyword" value=""></td>  <td width="14%"><span class="submit">
	        <input name="search2" type="button" class="btn" id="search2" onclick="javascript:doSearch('project_list.php')" value="Search"></span>
	        </td></tr>
	  <tr>
        ...
        <select name="orderby" class="table_text1"  id="orderby">
                  ...
                  <option value="asset_name" <?php echo ($orderby=='project_name')?"selected":""; ?>>Project Name</option>
                  </select>
...
<tr>
  ...
  <td align="right" nowrap="nowrap"> Limit :
    <select name="limit"   id="limit" onchange="javascript:doSearch('project_list.php');" style="width:100px;">
...
<table width="100%">
  <tr>
  &nbsp;&nbsp;<input name="add_new" type="button" class="btn" id="search" onclick="getpage('project.php','page')" value="Add New">
    ...
    <table class="table" width="100%">
      <thead>
        <tr class="ewTableHeader">
          ...
          <th nowrap="nowrap" class="Odd">Project ID</th>
          <th nowrap="nowrap" class="Odd">Project Name</th>
          <th nowrap="nowrap" class="Odd">Project Description</th>
          <th nowrap="nowrap" class="Odd">Timeline</th>
          <th nowrap="nowrap" class="Odd">Client</th>
          <th nowrap="nowrap" class="Odd">Project Status</th>
          <th nowrap="nowrap" class="Odd">Prioritization</th>
           <th nowrap="nowrap" class="Odd">Created User</th>
          <th nowrap="nowrap">Date Created</th>
          <th nowrap="nowrap">Date Started</th>
          <th nowrap="nowrap">Proposed Delivery Date </th>
          <th nowrap="nowrap">&nbsp;</th>
        </tr>
      </thead>
      ...
      <?php 
 		 ...
 		 for($i=0; $i<$skip; $i++){
			...
			$client_name=$dbobject->getitemlabel('client','client_id',$client_id,'client_name');
			?>
      <tr <?php echo ($k%2==0)?"class='treven'":"class='trodd'"; ?> >
        <td nowrap="nowrap" align="center"><?php echo $k+$skip;?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["project_id"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["project_name"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["project_desc"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["timeline"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $client_name;?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $status_name;?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $prioritization_name;?></td>
        ...
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo $row["date_started"];?></td>
        <td nowrap="nowrap" style="border-right:0px; border-left:0px"><?php echo ( $row["date_closed"] == '0000-00-00 00:00:00')? '-' : $row["date_closed"];?></td>
        
        <td nowrap="nowrap"><a href="javascript:getpage('project.php?op=edit&amp;project_id=<?php echo $row["project_id"];?>','page')"><font color="#33CC00">Edit</font></a>&nbsp;|&nbsp;<a href="javascript:getpage('deliverables_list.php?op=new&amp;project_id=<?php echo $row["project_id"];?>','page')"><font color="#0000FF">Deliverables</font></a></td>
